...
def iterative_solver(csv_path, T, delta, tau, z, Binary = False):
    """
    ...
    for t_fixed in range(len(T) - 1):
        ...
        for p in P:
            if t_fixed == 0:
                Delta[p] = int(np.clip(np.random.normal(mu[p], sigma_std[p]), 1, delta))
...
def evaluate_offline_objective_over_iterations(var_solutions, mu, sigma, beta, gamma, delta, T):
    """Evaluate offline objective using true mu and recorded variable solutions."""
    offline_I, offline_S, offline_W, offline_Obj = [], [], [], []

    for vars_iter in var_solutions:
        x, w, s, y = vars_iter['x'], vars_iter['w'], vars_iter['s'], vars_iter['y']

        I = sum(gamma.get((p, i, l), 0.0) * y.get((p, i, l), 0.0)
                for p in mu for l in range(1, sigma[p] - 1) for i in range(0, l))
        alpha = {(p, j): 50 * (mu[p] + 1 - j) * (mu[p] >= j) for p in mu for j in range(1, len(T))}
        S = sum(alpha.get((p, j), 0.0) * (1 - s.get((p, j), 0.0)) for p in mu for j in range(1, delta))
        W = sum(beta.get((p, t), 0.0) * (1 - w.get((p, t), 0.0)) for p in mu for t in range(sigma[p]))

        offline_I.append(I)
        offline_S.append(S)
        offline_W.append(W)
        offline_Obj.append(I + S + W)

    return offline_I, offline_S, offline_W, offline_Obj

def plot_metrics_vs_time(t_values, W_vals, I_vals, S_vals, Obj_vals, offline_I, offline_S, offline_W, offline_Obj):
    """Plot iterative vs offline evaluated objectives."""
    plt.figure(figsize=(12, 7))

    plt.plot(t_values, offline_I, label="I (Offline Eval)", linestyle=":")
    plt.plot(t_values, offline_S, label="S (Offline Eval)", linestyle=":")
    plt.plot(t_values, offline_W, label="W (Offline Eval)", linestyle=":")
    plt.plot(t_values, offline_Obj, label="Total Obj (Offline Eval)", linestyle="--")

    plt.xlabel("t_fixed (Time Step)")
    plt.ylabel("Objective Component Value")
    plt.title("Iterative vs Offline-Evaluated Objective Components")
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    plt.show()
